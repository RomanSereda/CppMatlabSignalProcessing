...
NoJitter::NoJitter(const double jitter_value, const double max_value, const int jitter_len)
	: mJitterLen(jitter_len), mMaxValue(max_value), mJitterValue(jitter_value) {

	mValues.reserve(jitter_len);
}

void NoJitter::compute(double* values, const int values_len, const int post_running_average_window) {
	srand(time(NULL));
...
void NoJitter::toNaN(double* values, const int values_len) {
	for (size_t j = 0; j < values_len; j++) {
...
void NoJitter::tofix(double* values, const int values_len) {
	int jitter_begin = -1;
